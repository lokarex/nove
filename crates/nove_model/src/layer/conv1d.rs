...
///
/// # Notes
/// * The `Conv1d` is now only created by the `Conv1dBuilder`.
///
/// # Fields
/// * `weight` - The weight tensor with shape [out_channels, in_channels/groups, kernel_size].
...
impl Conv1d {
    ...
    ///
    /// # Returns
    /// * `Tensor` - The weight tensor with shape [out_channels, in_channels/groups, kernel_size].
...
impl Model for Conv1d {
    ...
    /// Apply the 1D convolution layer to the input tensor.
    ///
    /// # Arguments
    /// * `input: Tensor` - The input tensor with shape [batch_size, in_channels, width].
    ///
    /// # Returns
    /// * `Ok(Tensor)` - The output tensor with shape [batch_size, out_channels, out_width] if successful.
...
impl Display for Conv1d {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "conv1d.{}(in_channels={}, out_channels={}, kernel_size={}, stride={}, padding={}, dilation={}, groups={}, bias_enabled={})",
            self.id,
            self.in_channels,
            self.out_channels,
            self.kernel_size,
...
impl Conv1dBuilder {
    ...
    /// Build the 1D convolution layer.
    ///
    /// # Returns
    /// * `Ok(Conv1d)` - The built 1D convolution layer.
    /// * `Err(ModelError)` - The error when building the 1D convolution layer.
    ///
    /// # Examples
    /// ```
    /// use nove::model::layer::Conv1dBuilder;
    ...
    pub fn build(&self) -> Result<Conv1d, ModelError> {
        ...
        if self.stride == 0 {
            return Err(ModelError::InvalidArgument(
                "stride in Conv1dBuilder must be greater than 0".to_string(),
            ));
        }
        if self.dilation == 0 {
            return Err(ModelError::InvalidArgument(
        ...
        let fan_in = (in_channels / self.groups) * kernel_size;
        let std = (2.0 / fan_in as f32).sqrt();

        let weight = Tensor::randn(
            0.0,
            std,
            &Shape::from_dims(&[out_channels, in_channels / self.groups, kernel_size]),
            &self.device,
            self.grad_enabled,
        )?
        .to_dtype(&self.dtype)?
        .require_name(&format!("conv1d.{}.weight", id))?;

        let bias = if self.bias_enabled {
            let bias = Tensor::zeros(
                &Shape::from_dims(&[out_channels, 1]),
                &self.dtype,
                &self.device,
                self.grad_enabled,
            )?
            .to_dtype(&self.dtype)?
            .require_name(&format!("conv1d.{}.bias", id))?;
            Some(bias)
        } else {
            None
        };
