...
///     .unwrap();
///
/// let input = Tensor::randn(0.0, 1.0, &Shape::from(&[32, 64, 28, 28]), &nove::device::candle::cpu().unwrap(), false).unwrap();
///
/// // Training mode
...
///
/// # Notes
/// * The `BatchNorm2dBuilder` must be created using [`BatchNorm2dBuilder::new()`] with required `num_features` argument.
