...
/// - g_t' is the gradient with weight decay applied (only when λ > 0)
    ...
    fn step(&mut self) -> Result<Self::StepOutput, OptimizerError> {
        ...
        for adam_param in &mut self.params {
            ...
            let grad_with_decay = if self.weight_decay > 0.0 {
                let param_with_decay = adam_param.param.affine(self.weight_decay, 0.0)?;
                grad.add(&param_with_decay)?
            } else {
                grad
            };

            // Update first moment: m_t = β₁·m_{t-1} + (1-β₁)·g_t' (using gradient with weight decay)
            let m_update = grad_with_decay.affine(1.0 - self.beta1, 0.0)?;
            ...
            // Bias-corrected estimates: m̂_t = m_t/(1-β₁^t), v̂_t = v_t/(1-β₂^t)
